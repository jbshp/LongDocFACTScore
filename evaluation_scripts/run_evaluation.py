import pandas as pd
import numpy as np
from utils.correlations import fleiss_kappa, krippendorff_alpha, interval_metric, kendal_tau_matrix
...
parser.add_argument("--dataset", choices=["pubmed_longdocfactscore", "arxiv_longdocfactscore","pubmed_longeval"],help="data set to run evaluation on")
...
def load_dataframes(dataset):
    if dataset == "arxiv_longdocfactscore":
        df_1 = pd.read_csv("./data/human_eval_results/LongDocFACTScore/arxiv_reviewer_1.csv")
        df_2 = pd.read_csv("./data/human_eval_results/LongDocFACTScore/arxiv_reviewer_2.csv")
        df_3 = pd.read_csv("./data/human_eval_results/LongDocFACTScore/arxiv_reviewer_3.csv")
        raw_path = "./data/raw_data/LongDocFACTScore/arxiv_test.json"
        return [df_1, df_2, df_3], raw_path
    if dataset == "pubmed_longdocfactscore":
        df_1 = pd.read_csv("./data/human_eval_results/LongDocFACTScore/pubmed_reviewer_1.csv")
        df_2 = pd.read_csv("./data/human_eval_results/LongDocFACTScore/pubmed_reviewer_2.csv")
        df_3 = pd.read_csv("./data/human_eval_results/LongDocFACTScore/pubmed_reviewer_3.csv")
        raw_path = "./data/raw_data/LongDocFACTScore/pubmed_test.json"
        return [df_1, df_2, df_3], raw_path
    if dataset == "pubmed_longeval":
        with open('./data/raw_data/LongEval/pubmed_summary_src_doc_data.json','r') as f:
        ...
        for article_id in article_ids:
            ...
            for idx, summary_method in enumerate(summary_methods):
                ...
                result[f"model_{idx}_factuality"] = np.mean(
                    [score_array['score'] for score_array in scores if score_array['story']==article_key][0]
                )
            result["article"] = clean_abstract(summaries[article_key]['source_doc'])
            result["ref"] = clean_abstract(summaries[f'article_{article_id}_human']['summary'])
...
def get_questeval_row(df,col_input,col_output,tgt_col): 
    inputs = list(df[col_input])
    outputs = list(df[tgt_col])
    df.loc[:,col_output] = test_questeval(
        hypothesis=inputs,
        sources=outputs)     
    return df
    ...
    with open(os.path.join(path,'data-dev.jsonl'),'w') as f:
        ...
        for ii, row in df.iterrows():
            ...
            for sent in pred_sents:   
                ...
                dict_ = {'claim':sent,'id':row['id'],'label':'CORRECT','text':row['article']}
...
def load_raw_data(df,raw_path):
    ...
    for idx, row in df.iterrows():
        article = [a for a in data if (a['article_id']==row['id'].strip() or (row['id'] in a['article_id']))][0] 
...
def get_scores_from_metrics(df,src_cols):
    df =  update_df_with_metric_scores(df,get_rouge_row,metric_name="rouge",src_cols=src_cols,tgt_col="article")
    df =  update_df_with_metric_scores(df,get_bert_score_row,metric_name="bertscore",src_cols=src_cols,tgt_col="article")
    df =  update_df_with_metric_scores(df,get_bartscore_row,metric_name="bartscore_src_hyp",tgt_col="article",switch_tgt=True,src_cols=src_cols)
...
#     df =  update_df_with_metric_scores(df,get_bartscore_row,metric_name="bartscore_recall",tgt_col="ref",switch_tgt=True,src_cols=src_cols)
    df =  update_df_with_metric_scores(df,get_ldfacts_row,metric_name="ldfacts_src_hyp",tgt_col="article",switch_tgt=True,src_cols=src_cols)
    df =  update_df_with_metric_scores(df,get_questeval_row,metric_name="questeval",tgt_col="article",switch_tgt=True,src_cols=src_cols)
    df = update_df_with_metric_scores(df,get_factcc_row,metric_name="factcc",src_cols=src_cols,tgt_col="article")
    return df
